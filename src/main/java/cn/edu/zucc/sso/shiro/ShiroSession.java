 ...
 * shiro 的 session 管理
 * 自定义session规则，实现前后端分离，在跨域等情况下使用token 方式进行登录验证才需要，否则没必须使用本类
 ...
 * <p>
 * 参考地址：https://www.liangzl.com/get-article-detail-131694.html
...
public class ShiroSession extends DefaultWebSessionManager {
    ...
    protected Serializable getSessionId(ServletRequest request, ServletResponse response) {
        ...
        HttpServletRequest toHttp = WebUtils.toHttp(request);
        String sessionId = toHttp.getHeader(AUTH_TOKEN);
        if(StringUtils.isEmpty(sessionId)){
            // 如果请求头中没有则尝试从参数中获取
            sessionId =  toHttp.getParameter(AUTH_TOKEN);
        }
